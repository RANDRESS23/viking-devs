        ...
        <Container>
          <div className="flex flex-col items-center text-center max-w-2xl mx-auto">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-heading font-semibold mt-6 text-[#171717]">
              Encuentra el plan ideal para tus <span className="text-[#ff5500] font-bold italic">proyectos digitales</span>
            </h2>
  ...
  return (
    <div
      className={cn(
        ...
        plan.title === 'Páginas Web' && 'border-[#ff5500]'
      )}
    >
      {plan.title === 'Páginas Web' && (
        <div className="absolute top-1/2 inset-x-0 mx-auto h-10 md:h-16 -rotate-45 w-full bg-[#ff5500] rounded-2xl lg:rounded-3xl blur-[8rem] z-10"></div>
      )}

      {plan.title === 'Páginas Web' && (
        <span className="absolute top-6 -right-11 text-xs py-1 px-12 text-white font-semibold rotate-45 bg-[#ff5500] z-10">POPULAR</span>
      )}
      ...
      <div className="p-4 md:p-8 flex rounded-t-2xl lg:rounded-t-3xl flex-col items-start w-full relative">
        ...
        <h3 className="relative mt-3 text-3xl font-medium md:text-5xl">
          ...
          {plan.title === 'Sitios Web Dinámicos' && (
            <span className='absolute -bottom-3 md:-bottom-1 -right-12 -rotate-[15deg] px-3 py-1 rounded-md font-light text-xs bg-[#ff5500] text-white'>Apartir de:</span>
          )}
        </h3>
        ...
        {plan.features.map((feature, index) => (
          <div key={index} className="flex items-center justify-start gap-2">
            <div className="flex items-center justify-center">
              <CheckIcon className="size-5 text-[#ff5500]" />
            </div>
            <span>{feature}</span>
          </div>
        ))}
